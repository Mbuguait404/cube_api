import { Injectable } from '@nestjs/common';
import { InjectModel } from '@nestjs/mongoose';
import { Model } from 'mongoose';
import {
  FeedbackSubmission,
  FeedbackSubmissionDocument,
} from './schemas/feedback-submission.schema';
import { CreateFeedbackSubmissionDto } from './dto/create-feedback-submission.dto';

@Injectable()
export class FeedbackSubmissionsService {
  constructor(
    @InjectModel(FeedbackSubmission.name)
    private feedbackModel: Model<FeedbackSubmissionDocument>,
  ) {}

  async create(
    dto: CreateFeedbackSubmissionDto,
  ): Promise<FeedbackSubmissionDocument> {
    return this.feedbackModel.create(dto);
  }

  async findAll(day?: string): Promise<FeedbackSubmissionDocument[]> {
    const filter = day ? { day } : {};
    return this.feedbackModel.find(filter).sort({ createdAt: -1 }).exec();
  }

  async countByDay(): Promise<{ day: string; count: number }[]> {
    return this.feedbackModel.aggregate([
      { $group: { _id: '$day', count: { $sum: 1 } } },
      { $project: { _id: 0, day: '$_id', count: 1 } },
      { $sort: { day: 1 } },
    ]);
  }

  async deleteOne(id: string): Promise<{ message: string }> {
    await this.feedbackModel.findByIdAndDelete(id);
    return { message: 'Deleted successfully' };
  }
}
